  ...
  async function renderMarkdown(content) {
    ...
    if (marked) {
      try {
        ...
        try { window.__editorLogs = window.__editorLogs || []; window.__editorLogs.push({evt: 'renderMarkdown-html', len: typeof html === 'string' ? html.length : null, sample: typeof html === 'string' ? html.slice(0,800) : null}); } catch(e){}
        try { console.log('renderMarkdown: HTML generated by marked', html); } catch(e){}
        return html;
      } catch (err) {
